...
def simulate_content_summarization():
    ...
    blockchain.submit_work(intent_id, AGENT_B, result_ipfs)
    ...
    # Simple verification: check that the summary mentions key concepts
    ...
    if verified:
        ...
        blockchain.verify_work(intent_id, AGENT_A, approved=True)
    ...
    print(f"💰 Agent A new balance: {blockchain.get_balance(AGENT_A) - amount_usdc} USDC (spent {amount_usdc} USDC)")
