...
contract AgentWorkEscrowZKTest is Test {
    ...
    function test_release_without_zk() public {
        ...
        (
            ...
            string memory pu2, string memory d2
        ) = escrow.intents(id);
    ...
    function test_setZKVerifier() public {
        ...
        assertEq(address(escrow.zkVerrier()), address(0));

        vm.prank(owner);
        escrow.setZKVerifier(address(verifier));
        assertEq(address(escrow.zkVerrier()), address(verifier));
    }
    ...
    function test_submitProofWithZK() public {
        ...
        (
            ...
            string memory pu3, string memory d3
        ) = escrow.intents(id);
    ...
    function test_tesspay_auto_payment_on_valid_zk_proof() public {
        ...
        assertEq(finalFeeBalance - initialFeeBalance, expectedFee);

        // Contract balance should be zero (all paid out)
        assertEq(usdc.balanceOf(address(escrow)), 0);

        // Verify that TessPay event was emitted (we can check this via vm.expectEmit if needed)
