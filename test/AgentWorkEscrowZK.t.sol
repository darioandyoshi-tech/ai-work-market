...
/// @notice Mock USDC for testing. Plain contract (no EIP712 inheritance) to avoid
/// a solc via_ir codegen bug that corrupts assertEq comparisons in complex test contracts.
contract MockUSDC {
...
contract MockVerifier {
    function verifyProof(uint[2] calldata _pA, uint[2][2] calldata _pB, uint[2] calldata _pC, uint[2] calldata _pubSignals) public pure returns (bool) {
        // For our test values: return true if pubSignals[0] == 1 (valid) and pubSignals[1] == 9 (commitment)
...
contract AgentWorkEscrowZKTest is Test {
    ...
    function test_createIntent() public {
        ...
        assertEq(disputeHash, bytes32(0));
    }
    ...
    function test_release_without_zk() public {
        ...
        // Buyer releases (no ZK proof submitted, so it passes)
        vm.prank(buyer);
        escrow.release(id);
        ...
        (
            ...
            bytes32 wh2, AgentWorkEscrowZK.Status st2,
            bytes32 pu2, bytes32 d2
        ) = escrow.intents(id);
    ...
    function test_submitProofWithZK() public {
        ...
        // Seller submits ZK proof (valid signals [1,9] -> TessPay auto-releases)
        vm.prank(seller);
        ...
        (
            ...
            bytes32 wh3, AgentWorkEscrowZK.Status st3,
            bytes32 pu3, bytes32 d3
        ) = escrow.intents(id);
        // Valid proof triggers TessPay auto-payment -> status is Released
    ...
    function test_release_with_zk_notConfigured() public {
        ...
        // Submit proof with ZK: should revert because verifier not configured
        vm.prank(seller);
    ...
    function test_tesspay_auto_payment_on_valid_zk_proof() public {
        ...
        // Fee accrues to accumulatedFees (withdrawn later by feeRecipient via withdrawFees)
        // NOTE: solc via_ir codegen bug corrupts assertEq on accumulatedFees in this complex
        // test contract (compares against phantom 0). Verified correct via minimal test:
        // the contract accrues exactly expectedFee. Use a direct comparison to avoid the bug.
        uint256 accruedFees = escrow.accumulatedFees();
        if (accruedFees != expectedFee) {
            revert("fee mismatch");
        }

        // Contract holds the fee (expectedFee) until withdrawFees() is called
        assertEq(usdc.balanceOf(address(escrow)), expectedFee);
    }
}
