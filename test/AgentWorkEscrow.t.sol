...
contract AgentWorkEscrowTest is Test {
    ...
    function testCreateIntentStoresWorkMetadataAndFundsEscrow() public {
        ...
        assertEq(storedProofHash, bytes32(0));
        assertEq(storedDisputeHash, bytes32(0));
    ...
    function testSellerCanSubmitProofSetsReviewDeadline() public {
        uint256 id = _createIntent();
        _submitProof(id);
        (, , , , , , uint256 reviewDeadline, , , AgentWorkEscrow.Status status, bytes32 proofHash, ) = escrow.intents(id);
        assertEq(uint256(status), uint256(AgentWorkEscrow.Status.ProofSubmitted));
        assertEq(proofHash, keccak256(bytes("ipfs://proof")));
    ...
    function testBuyerAndSellerCanDisputeWithReason() public {
        ...
        (, , , , , , , , , AgentWorkEscrow.Status status1, , bytes32 disputeHash1) = escrow.intents(id1);
        assertEq(uint256(status1), uint256(AgentWorkEscrow.Status.Disputed));
        assertEq(disputeHash1, keccak256(bytes("ipfs://buyer-reason")));

        uint256 id2 = _createIntent();
        _submitProof(id2);
        vm.prank(seller);
        escrow.dispute(id2, "ipfs://seller-reason");
        (, , , , , , , , , AgentWorkEscrow.Status status2, , bytes32 disputeHash2) = escrow.intents(id2);
        assertEq(uint256(status2), uint256(AgentWorkEscrow.Status.Disputed));
        assertEq(disputeHash2, keccak256(bytes("ipfs://seller-reason")));
    }
    ...
    function testOwnerCanResolvePartialDisputeWithFee() public {
        ...
        // Buyer started with amount*10, spent amount on escrow, gets buyerAmount back
    ...
    function testURIValidationEdgeCases() public {
        // Mint enough USDC for the multiple intents created in this test
        usdc.mint(buyer, amount * 10);
        vm.startPrank(buyer);
        usdc.approve(address(escrow), type(uint256).max);
        ...
        // NOTE: workURI uses _validateBasicURI (non-empty, <=512 bytes), NOT the ipfs://
        // prefix check. So non-IPFS strings like "IPFS://valid" and "ipfs:/valid" are VALID
        // workURIs and do NOT revert. Only empty and too-long URIs revert.
        vm.stopPrank();
    }
