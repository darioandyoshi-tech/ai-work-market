...
| Accounting | Reconciliation | Define how platform reconciles `accumulatedFees`, outstanding principal, Stripe revenue, manual fulfillment, and treasury withdrawals. | Reconciliation script/report format. |
| API hardening | Production secrets and auth | `AWM_DELIVERY_TOKEN`, `STRIPE_SECRET_KEY`, and `STRIPE_WEBHOOK_SECRET` must be production secrets in Vercel, rotated from any dev values. Avoid `access_token` query auth for production protected resources or mark it deprecated because URLs leak in logs/referrers. | Vercel env audit, token rotation date, endpoint behavior test. |
