...
| Webhook persistence | Durable fulfillment trail | `api/stripe-webhook.js` currently logs verified events but does not persist fulfillment state. For production fulfillment, add durable storage or an explicit manual ops workflow that cannot lose purchases if logs expire. | DB/table or manual ops SOP with Stripe dashboard fallback. **Current: Local JSON store is ephemeral on Vercel. Use Stripe Dashboard as source of truth for manual fulfillment.** |
